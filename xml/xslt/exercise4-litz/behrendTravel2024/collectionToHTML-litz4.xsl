...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
   ...
   <xsl:template match="letter" mode="toc">
       <tr>
           <td><xsl:apply-templates select="descendant::headLine ! normalize-space() => distinct-values() => sort() => string-join(',')"/></td><!--first column data cell: to hold the date of the letter-->
           <td><xsl:apply-templates select="descendant::person ! normalize-space() => distinct-values() => sort() => string-join(',')"/> </td><!--second column data cell: to hold a sorted, string-joined list of persons mentioned. -->
           <td><xsl:apply-templates select="descendant::location ! normalize-space() => distinct-values() => sort() => string-join(',')"/></td><!--third column data cell: to hold a sorted, string-joined list of locations mentioned.-->
