...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
   ...
   <xsl:template match="letter" mode="toc">
       <tr>
           <td><xsl:apply-templates select="descendant::headLine"/></td><!--first column data cell: to hold the date of the letter-->
           <td>
               <xsl:apply-templates select="descendant::person ! normalize-space() => distinct-values() => sort() => string-join(',')"/>
               <xsl:apply-templates select="descendant::location ! normalize-space() => distinct-values() => sort() => string-join(',')"/></td><!--second column data cell: to hold a sorted, string-joined list of persons mentioned. -->
           <td><xsl:apply-templates select="string-join(descendant::location,',')">        </xsl:apply-templates></td><!--third column data cell: to hold a sorted, string-joined list of locations mentioned.-->
    ...
    <xsl:template match="letter">
        <section class="readingView" id="date-{when}">
            <xsl:apply-templates />
        </section>
